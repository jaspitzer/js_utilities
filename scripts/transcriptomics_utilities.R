...
hm_js <- function(df,
  ...
  if (normalise_params) { # normalise params
    df_input <- df_input %>%
      ...
      } %>%
      {
        if (norm_method == "max") {
          mutate(., across(
            .cols = where(is.numeric) & !excluded_vars,
            function(x) {
              (x - min(x)) / (max(x) - min(x))
            }
          ))
  ...
  if (normalise_samples) { # normalize samples
    df_input <- df_input %>%
      ...
      pivot_wider(
        names_from = ifelse(nchar(id_col) > 0, id_col, names(df)[1]),
        values_from = "val"
      ) %>%
      {
        if (norm_method == "zscore") {
  ...
  if (is.numeric(custom_threshold) & norm_method == "zscore") {
    ...
    thres <- df_plot$val %>%
      quantile(probs = outlier.threshold) %>%
      round(2)
    df_plot <- df_plot %>%
      mutate(val = case_when(
        val > thres ~ thres,
  ...
  if (.plot) {
      ...
      geom_tile() +
      {
        if (outlier.removal | (is.numeric(custom_threshold) & norm_method == "zscore")) {
          scale_fill_gradient2(
            high = color_code[1], low = color_code[2],
            breaks = fill_labels,
            labels = fill_names
          )
        } else {
    ...
    if (add_annotation) {
      ...
      if (length(anno_col) > 1) {
        ...
        anno_cols <- df_plot %>%
          dplyr::select(-c(SAMPLE, params, val)) %>%
          names()
      }
      ...
      plot <- wrap_plots(plot_list,
        heights = c(if (clust_samples & show_dend_samples) {
          0.3
        }, rep(0.05, length(anno_cols)), 1),
